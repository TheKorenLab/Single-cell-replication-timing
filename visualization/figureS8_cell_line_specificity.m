...
figureS8 = figure;
set(figureS8, 'Units', 'inches', 'Position', [25 9 6.5 6.5])
...
examples = struct('Chr', {1 2 1 4}, 'X', {[194.5 200] [180 184] [79 85] [175 180]}, ...
    ...
    'origins', {[196.1362 198.5757] 182.1423 [81.5372 81.9271] 177.8});
...
for e = 1:4
    ...
    for o = 1:length(examples(e).origins)
        plot(examples(e).origins(o) * ones(1, 2), [-2 2], 'k', 'LineWidth', 0.7, ...
    ...
    for l = 1:length(examples(e).samples)
        ...
        imagesc(genome_windows{Chr}(:, 3) ./1e6, 1:num_cells, r', 'AlphaData', ~isnan(r'), ...
            'Parent', parent)
        set(parent, 'YDir', 'reverse', 'XLim', X, 'YLim', [0.5 num_cells+0.5], ...
        ...
        for o = 1:length(examples(e).origins)
            plot(examples(e).origins(o) * ones(1, 2), [0.5 num_cells+0.5], 'k', 'LineWidth', 0.7, ...
                'HandleVisibility', 'off', 'Parent', parent)
        end
        ...
        ylabel(parent, cell_line_names{s_index})
...
for b = [1 4 7 2 5 8 3 6 9]
    ...
    plot(0, 0, 'LineWidth', 2, 'Color', c, 'DisplayName', cell_line_names{b}, 'Parent', parent)
end

legendD = legend(panels(4).bulk);
set(legendD, 'Units', 'inches', 'Position', [3.65 0.2 2.5 0.5], ...
